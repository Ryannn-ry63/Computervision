...
class Painter:
    def __init__(self, seg_net_index, training_path=TRAINING_PATH, save_path=None, device='cuda:0'):
        self.root_split_path = training_path
        if not self.root_split_path.endswith('/'):
            self.root_split_path += '/'
        self.save_path = save_path or (self.root_split_path + "painted_lidar/")
        ...
        if seg_net_index == 0:
            ...
            if str(device).lower() == 'cpu':
                config_file = './mmseg/configs/deeplabv3plus/deeplabv3plus_r101-d8_512x1024_80k_cityscapes_cpu.py'
            else:
    ...
    def get_score(self, idx, left):
        ...
        if self.seg_net_index == 0:
            ...
            filename = self.root_split_path + left + ('%s.png' % idx)
            result = self.inference_segmentor(self.model, filename)
    ...
    def run(self, num_image=7481, sample_indices=None):
        if sample_indices is None:
            sample_indices = ["%06d" % idx for idx in range(num_image)]
...
def parse_args():
    parser = argparse.ArgumentParser(description='Paint KITTI LiDAR points with image semantic scores.')
    parser.add_argument('--seg-net', type=int, default=SEG_NET, choices=[0, 1, 2],
                        help='0=deeplabv3, 1=deeplabv3plus, 2=hma')
    parser.add_argument('--training-path', default=TRAINING_PATH,
                        help='KITTI training directory containing image_2, image_3, velodyne and calib.')
    parser.add_argument('--save-path', default=None, help='Directory for painted .npy outputs.')
    parser.add_argument('--sample-idx', default=None,
                        help='Single sample id such as 000008. If omitted, runs from 000000 to num-image-1.')
    parser.add_argument('--num-image', type=int, default=7481, help='Number of images for batch painting.')
    parser.add_argument('--device', default='cuda:0', help='Device for mmseg models.')
    return parser.parse_args()

if __name__ == '__main__':
    args = parse_args()
    sample_indices = [args.sample_idx] if args.sample_idx else None
    painter = Painter(args.seg_net, training_path=args.training_path, save_path=args.save_path, device=args.device)
    painter.run(num_image=args.num_image, sample_indices=sample_indices)
